...
AsyncDelay delay_1000ms; // Create an Async delay object called delay_10ms. The name is arbitrary to the actual time period set
...
// If Switch is on, Increment Count and Report it
  if(switch_state)
  {
    if (delay_1000ms.isExpired())
    {
      ...
      append_to_bundle_key_value(bndl, "countIs", count);
